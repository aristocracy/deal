<template>
  <uni-popup ref="popup" type="bottom" @touchmove.stop.prevent="()=>{return}">
    <view class="picker-box" :style="{height:screenHeight+'px'}">
      <view class="picker-close" @click="onClose()">
		  <uni-icons type="close" size="30" color="#d4237a"></uni-icons>
      </view>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      onSelectConfirm() {
        let areainfo={province:{name:this.province.name,id:this.province.id},city:{name:this.city.name,id:this.city.id},district:{name:this.district.name,id:this.district.id}};
		this.$emit("areainfo",areainfo);
		this.$refs.popup.close()
      }
